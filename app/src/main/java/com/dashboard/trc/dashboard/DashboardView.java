...
public class DashboardView extends View {
    ...
    private float mRealTimeTextSize = spToPx(15); // 实时值
    private int mRealTimeTextColor = 0xFF14b38a; // 实时值
    private int mStripeWidth; // 色条宽度
    private StripeMode mStripeMode = StripeMode.INNER;
    private int mBigSliceRadius; // 较长刻度半径 长度
    private int mBigSliceWidth = 2; // 较长刻度宽度 dp
    ...
    public DashboardView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mModeType = a.getInt(R.styleable.DashboardView_stripeMode, 1);
    ...
    private String[] getMeasureNumbers() {
        ...
        for (int i = 0; i <= mBigSliceCount; i++) {
            if (i == 0) {
                ...
                /**
                 * 如果最小值为负数 最小值除每段值  等于她的负数区间的段数
                 *  例如 下
                 *  |-----|-----|-----|-----|-----|-----|-----|
                 * -60   -40   -20    0     20    40    60    80
                 */
                if (mMaxValue - mMinValue >= 10) {
                    strings[i] = String.valueOf(((mMaxValue - mMinValue) / mBigSliceCount) * i + mMinValue);
                } else {
                    float num = ((float) (mMaxValue - mMinValue) / (float) mBigSliceCount) * i + (float) mMinValue;
                    try {
                        if (String.valueOf(num).substring(3, 4).equals("0")) {
                            strings[i] = String.valueOf(num).substring(0, 3);
                        } else {
                            strings[i] = String.valueOf(num).substring(0, 4);
                        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        mPaintArc.setStrokeWidth(dpToPx(mBigSliceWidth));
        //绘制刻度盘
        for (int i = 0; i <= mBigSliceCount; i++) {
            //绘制大刻度
            ...
            if (i == 0 || i == mBigSliceCount) {
                ...
                canvas.drawText(trimFloat(Float.parseFloat(number)), numberPoint[0], numberPoint[1] + mRectMeasures.height(), mPaintText);
            }
        }
        ...
        // 绘制读数
        mPaintValue.setColor(pointerColor);
        canvas.drawText(trimFloat(mRealTimeValue), mCenterX,
                mCenterY + mCircleRadius + dpToPx(2) + dpToPx(65), mPaintValue);
    }
    ...
    private int getRealColor(float mSweepAngle) {
        if (mStripeHighlight != null) {
            for (int i = 0; i < mStripeHighlight.size(); i++) {
                HighlightCR highlightCR = mStripeHighlight.get(i);
                if (highlightCR.getColor() == 0 || highlightCR.getSweepAngle() == 0)
                    continue;
                float start = highlightCR.getStartAngle();
                float end = highlightCR.getStartAngle() + highlightCR.getSweepAngle();
                if (end > getStartAngle() + getSweepAngle()) {
                    end = getStartAngle() + getSweepAngle();
                }
                if (start <= mSweepAngle) {
                    if (end >= mSweepAngle) {
                        pointerColor = (highlightCR.getColor());
                    }
                }
    ...
    private float getAngleFromResult(float result) {
        if (result > mMaxValue)
            return (float) mMaxValue;
        return (float) (mSweepAngle * (result - mMinValue) / (mMaxValue - mMinValue) + mStartAngle);
    }
    ...
    public float getmRealTimeTextSize() {
        return mRealTimeTextSize;
    }

    public void setmRealTimeTextSize(float mRealTimeTextSize) {
        this.mRealTimeTextSize = mRealTimeTextSize;
        init();
    }

    public int getmRealTimeTextColor() {
        return mRealTimeTextColor;
    }

    public void setmRealTimeTextColor(int mRealTimeTextColor) {
        this.mRealTimeTextColor = mRealTimeTextColor;
        init();
    }
